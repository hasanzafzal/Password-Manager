...
bool PasswordManagerBackend::add_entry(const std::string& account_name, const std::string& password) {
    // If the account already exists, update its password in place rather
    // than appending a second node with the same account name (the old
    // behavior silently created duplicate entries every time you "added"
    // an account that was already saved).
...
bool PasswordManagerBackend::delete_entry(const std::string& account_name) {
    ...
    while (entries) {
        if (entries->account_name == account_name) {
            ...
            bst.remove(account_name);  // keep the BST index in sync with the deletion
            return true;
        }
        prev = entries;
        entries = entries->next;
